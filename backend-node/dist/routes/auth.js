...
router.post('/signup', async (req, res) => {
    try {
        ...
        const token = (0, jwt_1.createAccessToken)({ sub: newUser.email });
        res.status(200).json({
            access_token: token,
            token_type: 'bearer',
        });
...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = (0, jwt_1.createAccessToken)({ sub: user.email });
        res.status(200).json({
            access_token: token,
            token_type: 'bearer',
        });
...
router.post('/google', async (req, res) => {
    try {
        ...
        let userResult = await (0, database_1.query)('SELECT * FROM users WHERE email = $1', [email]);
        let user;
        ...
        const accessToken = (0, jwt_1.createAccessToken)({ sub: user.email });
        res.status(200).json({
